...
// of the now-removed Character Viewer for a dedicated, cleaned-up view. See data/pointer_map.md's
// "NPC/Villager relationship struct" section for the underlying derivation this all builds on.
//
// "Homogenized" means every row here (known fields AND generic "Unknown" gap rows alike) is
// guaranteed to fit within EVERY included villager's own real window, capped at the SMALLEST
// window any of them has (see HOMOGENIZED_WINDOW_SIZE below), not the largest. A known field that
// doesn't fit within that smallest window (e.g. Location didn't fit for this group before the
// computeWindowSize fix noted below) is simply left off rather than shown misleadingly for some
// columns and blank for others. Columns are labeled and ordered by the game's own Internal ID.
//
...
// the exact same source the Villagers window also reads from, so both tabs agree on
// offsets/names/IDs by construction.
...
// villagers_agent.js's identical constant/reasoning.
const FALLBACK_WINDOW_SIZE = 0x38;
const POLL_INTERVAL_MS = 500;

// Same technique as villagers_agent.js's computeWindowSize -- the gap to the CLOSEST LARGER offset
// ANYWHERE in the full (unfiltered) array (not just scanning forward from this position), since
// that's the real boundary to the next character's record regardless of which columns this
// particular tab happens to display (fixes a bug where Brandon/Jennifer's out-of-sequence offsets
// made Lou's window read 216 bytes instead of the true 56).
...
// out to the homogenized window -- same pattern as villagers_agent.js's buildRowTemplate,
...
function readField(base, field) {
    ...
    return addr.readS32(); // signed -- see villagers_agent.js's identical note
}
...
function poll() {
    ...
    // app/windows/marriage_candidates.py's _on_cell_batch for the Python side of this.
    const updates = [];
...
// app/windows/marriage_candidates.py -- same pattern as villagers_agent.js's dumpBytes.
...
function listenForAddLovePoints() {
    recv('addLovePoints', (message) => {
        addToField('Love Points', message.amount);
        listenForAddLovePoints();
    });
}
listenForAddLovePoints();
