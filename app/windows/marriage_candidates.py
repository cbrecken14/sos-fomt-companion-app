"""Marriage Candidates: a homogenized matrix over the 12 marriage-candidate villagers -- split out
of the now-removed Character Viewer for a clean, dedicated view (no non-candidate villagers, no
Harvest Sprites). Every row (known fields and unlabeled "Unknown" gap rows alike) is guaranteed to
fit within EVERY included villager's own real record, not just some of them. See
src/marriage_candidates_agent.js for the row/column derivation and data/pointer_map.md's
"NPC/Villager relationship struct" section for the underlying struct this all builds on.

Columns are labeled and ordered by the game's own Internal ID.

Same attach/cleanup plumbing as villagers.py: creates its own script on the app's one shared
Frida session (app/game_session.py), only needs the Save Data Base signal.
"""
...
AGENT_PATH = Path(__file__).resolve().parent.parent.parent / "src" / "marriage_candidates_agent.js"
...
# change forcing a full resend) -- see marriage_candidates_agent.js's poll(). Flashing thousands of
# cells at once is both meaningless and the real cost of the old per-cell-message loading lag, so
# bulk batches skip the flash effect entirely; normal small live-update batches still flash as
# before.
...
class MarriageCandidatesWidget(QWidget):
    ...
    def _attach_worker(self) -> None:
        try:
            ...
            agent_source = load_agent_source(AGENT_PATH, tables=("character_viewer",))
    ...
    def _add_love_points(self, amount: int) -> None:
        if self._script is not None:
            self._script.post({"type": "addLovePoints", "amount": amount})

    def _add_friendship(self, amount: int) -> None:
        if self._script is not None:
            self._script.post({"type": "addFriendship", "amount": amount})
    ...
    def _on_message(self, message, data) -> None:
        ...
        if message["type"] != "send":
            if message["type"] == "error":
                print(f"[marriage candidates] frida error: {message.get('description')}")
            return
        ...
        if kind == "schema":
            ...
            # Printed directly here (not marshaled through a Signal) -- this is plain console
            # output, not a Qt widget update, same as the frida-error print a few lines up.
    ...
    def _apply_updates(self, updates: list, suppress_flash: bool) -> None:
        for update in updates:
            item = self._items[update["rowIndex"]][update["colIndex"]]
            item.setText(str(update["raw"]))
    ...
    def _print_hex_dump(label: str, address: str, byte_values: list[int]) -> None:
        # Dumps a villager's raw bytes to the console for diffing before/after an in-game action.
        print(f"[marriage candidates] Dump: {label} @ {address} ({len(byte_values)} bytes)")
    ...
    def cleanup(self) -> None:
        ...
        session, not owned by this tab, so it must NOT be detached here (see animal_viewer.py's
        identical note)."""
        script = self._script
...
register(WindowSpec("marriage_candidates", "Marriage Candidates", MarriageCandidatesWidget, debug=True))
