"""Harvest Sprite Viewer: a matrix over the 7 Harvest Sprite records, split out from the
now-removed Character Viewer since sprites will likely have
different tracked properties than regular villagers -- their offsets are evenly spaced 0x50 apart,
a tighter/more uniform pattern than villagers, which supports that theory. See
src/harvest_sprite_viewer_agent.js and data/pointer_map.md's "NPC/Villager relationship struct"
section for the underlying derivation.

Starts as a blank exploration grid -- no fields confirmed yet for sprites specifically. Interact
with sprites in-game and watch which offset moves.
...
AGENT_PATH = Path(__file__).resolve().parent.parent.parent / "src" / "harvest_sprite_viewer_agent.js"
...
# change forcing a full resend) -- see harvest_sprite_viewer_agent.js's poll(). Flashing thousands
# of cells at once is both meaningless and the real cost of the old per-cell-message loading lag,
# so bulk batches skip the flash effect entirely; normal small live-update batches still flash as
# before.
...
class HarvestSpriteViewerWidget(QWidget):
    ...
    def _attach_worker(self) -> None:
        try:
            ...
            agent_source = load_agent_source(AGENT_PATH, tables=("harvest_sprite_viewer",))
    ...
    def _on_message(self, message, data) -> None:
        ...
        if message["type"] != "send":
            if message["type"] == "error":
                print(f"[harvest sprite viewer] frida error: {message.get('description')}")
            return
    ...
    def _apply_updates(self, updates: list, suppress_flash: bool) -> None:
        for update in updates:
            ...
            item.setText(str(calculated) if calculated else str(update["raw"]))
    ...
    def _print_hex_dump(label: str, address: str, byte_values: list[int]) -> None:
        # Dumps a sprite's raw bytes to the console for diffing before/after an in-game action.
        print(f"[harvest sprite viewer] Dump: {label} @ {address} ({len(byte_values)} bytes)")
...
register(WindowSpec("harvest_sprite_viewer", "Harvest Sprite Viewer", HarvestSpriteViewerWidget, debug=True))
