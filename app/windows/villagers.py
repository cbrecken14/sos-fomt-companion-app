...
out of the now-removed Character Viewer alongside Marriage Candidates (see marriage_candidates.py's
header comment for the shared rationale: every row, known or unknown, that fits within EVERY
included villager's own real record; Internal ID ordering/labeling). Includes Gotts and Saibara,
but their own carpenter/forge-job-specific bytes past the common region are excluded the same way
any other villager's non-shared tail would be -- some other, more tightly-packed villager in this
list sets the homogenized cutoff, not a Gotts/Saibara special case. See
src/villagers_agent.js's HOMOGENIZED_WINDOW_SIZE derivation.

Same attach/cleanup plumbing as marriage_candidates.py: creates its own script on the app's one
shared Frida session (app/game_session.py), only needs the Save Data Base signal.
"""
...
AGENT_PATH = Path(__file__).resolve().parent.parent.parent / "src" / "villagers_agent.js"
...
# change forcing a full resend) -- see villagers_agent.js's poll(). Flashing thousands of cells at
# once is both meaningless and the real cost of the old per-cell-message loading lag, so bulk
# batches skip the flash effect entirely; normal small live-update batches still flash as before.
...
class VillagersWidget(QWidget):
    ...
    def _on_message(self, message, data) -> None:
        ...
        if message["type"] != "send":
            if message["type"] == "error":
                print(f"[villagers] frida error: {message.get('description')}")
            return
    ...
    def _print_hex_dump(label: str, address: str, byte_values: list[int]) -> None:
        ...
        print(f"[villagers] Dump: {label} @ {address} ({len(byte_values)} bytes)")
...
register(WindowSpec("villagers", "Villagers", VillagersWidget, debug=True))
